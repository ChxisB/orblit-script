...
static void rejection_tracker(JSContext *ctx, JSValueConst promise,
                              ...
                              void *opaque) {
  (void)promise;
  OrblitScriptHost *host = (OrblitScriptHost *)opaque;
  if (!host) return;
...
OrblitScriptHost *orblit_script_new(void) {
  OrblitScriptHost *host = (OrblitScriptHost *)calloc(1, sizeof(OrblitScriptHost));
  if (!host) return NULL;
...
void orblit_script_free(OrblitScriptHost *host) {
  if (!host) return;
  free(host->pending_rejection);
...
int orblit_script_eval(OrblitScriptHost *host, const char *src,
...
int orblit_script_pump(OrblitScriptHost *host, char **err_out) {
...
void orblit_script_string_free(char *s) { free(s); }
...
#define ORBLIT_STR_(x) #x
#define ORBLIT_STR(x) ORBLIT_STR_(x)

const char *orblit_script_engine_version(void) {
  return ORBLIT_STR(QJS_VERSION_MAJOR) "." ORBLIT_STR(QJS_VERSION_MINOR) "." ORBLIT_STR(
