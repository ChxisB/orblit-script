...
class ScriptHost {
  ScriptHost() : _handle = native.orblitScriptNew() {
    if (_handle == nullptr) {
      throw StateError('Could not create a QuickJS runtime.');
    }
  }

  Pointer<native.OrblitScriptHostStruct> _handle;
  ...
  static String get engineVersion =>
      native.orblitScriptEngineVersion().cast<Utf8>().toDartString();
  ...
  String eval(String source, {String fileName = '<eval>'}) {
    ...
    return using((arena) {
      ...
      final threw = native.orblitScriptEval(
            _handle,
  ...
  int pumpMicrotasks() {
    ...
    return using((arena) {
      ...
      final ran = native.orblitScriptPump(_handle, error);
  ...
  void dispose() {
    ...
    native.orblitScriptFree(_handle);
    _handle = nullptr;
  }
  ...
  String _consume(Pointer<Char> pointer, {required bool isError}) {
    ...
    native.orblitScriptStringFree(pointer);
