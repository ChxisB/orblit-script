...
void main() {
  group('spawning from script', () {
    ...
    List<ScriptedThing> world() =>
        ScriptedThing.decodeAll(host.eval('__orblit_scene.describe()'));
    ...
    test('a frame runs the script step and then says what is there', () {
      ...
      final first =
          ScriptedThing.decodeAll(host.eval('__orblit_scene.frame(1.5)'));
      expect(first.single.position.x, 3);

      final second =
          ScriptedThing.decodeAll(host.eval('__orblit_scene.frame(4)'));
